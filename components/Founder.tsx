...
export function Founder({ t }: { t: Dictionary }) {
  ...
  const onFocus = (e: FocusEvent<HTMLElement>) => { if (e.target.matches(":focus-visible")) setHold(true); };
  return (
    <section ref={ref} id="founder" className="mx-auto max-w-7xl scroll-mt-28 px-6 py-16">
      <Reveal><figure className="grid overflow-hidden rounded-[2rem] border border-ink/10 bg-surface lg:grid-cols-[minmax(0,5fr)_minmax(0,7fr)]">
        ...
        <div className="flex flex-col justify-center p-8 sm:p-14"><Eyebrow>{founder.label}</Eyebrow><blockquote className="mt-6 text-3xl font-medium leading-snug tracking-tight text-ink sm:text-4xl">{founder.quote}</blockquote>
          <figcaption className="mt-8 flex flex-wrap gap-3" onFocus={onFocus} onBlur={() => setHold(false)}>{people.map((p, i) => { const on = i === index; return <button key={p.name} type="button" onClick={() => setIndex(i)} aria-pressed={on} className={cn("relative overflow-hidden rounded-2xl px-4 py-3 text-start transition-colors duration-300", on ? "bg-white shadow-sm" : "hover:bg-white/60")}><span className={cn("block font-semibold transition-colors", on ? "text-ink" : "text-ink/45")}>{p.name}</span><span className={cn("block text-sm transition-colors", on ? "text-muted" : "text-muted/60")}>{p.role}</span>{on && running && <span key={index} aria-hidden className="founder-progress absolute inset-x-0 bottom-0 h-0.5 origin-left bg-brand rtl:origin-right" />}</button>; })}</figcaption>
        </div>
      </figure></Reveal>
    </section>
  );
